...
for t in out_types:
    ...
    rows = ''.join([f"<li><a href='/{t}/{x['id']}/'>{x['name']}</a></li>" for x in items])
    hub_html = f"<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>STS2 {t.title()} Hub | STS2 Guide</title><meta name='description' content='{intro}'><link rel='canonical' href='https://slaythespire2guide.com/{t}/'><link rel='stylesheet' href='../style.css'></head><body><nav><div class='nav-logo'>STS2 Guide</div><ul class='nav-links'><li><a href='/'>Home</a></li><li><a href='/updates.html'>Updates</a></li></ul></nav><div class='section'><h1>{t.title()} Hub</h1><p class='section-sub'>{intro}</p><ul>{rows}</ul></div></body></html>"
